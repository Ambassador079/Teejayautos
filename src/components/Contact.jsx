...
const item    = { hidden: { opacity: 0, y: 38 }, show: { opacity: 1, y: 0, transition: { duration: 0.8, ease: [0.22, 1, 0.36, 1] } } }
...
export default function Contact() {
  ...
  const INFO = [
    { icon: <Phone size={16} />, label: 'Phone',    val: BRAND.phone,    href: `tel:${BRAND.phone}`,         sub: BRAND.phone2 },
    { icon: <MapPin size={16} />, label: 'Location', val: BRAND.location, href: null },
    { icon: <Mail size={16} />,  label: 'Email',    val: BRAND.email,    href: `mailto:${BRAND.email}` },
  ]

  return (
    <section id="contact" className="py-24 lg:py-36 bg-cream px-5 sm:px-8 xl:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14">
          ...
          {/* Info */}
          <motion.div variants={stagger} initial="hidden" animate={inView ? 'show' : 'hidden'} className="space-y-9">
            {INFO.map((row, i) => (
              <motion.div key={i} variants={item} className="flex gap-5 items-start">
                <div className="w-11 h-11 border border-gold/35 flex items-center justify-center text-gold flex-shrink-0">
                  {row.icon}
                </div>
                <div>
                  <div className="font-sans text-[9px] tracking-[0.38em] uppercase text-gold mb-1.5">{row.label}</div>
                  {row.href
                    ? <a href={row.href} className="font-serif text-[17px] text-ink hover:text-gold transition-colors">{row.val}</a>
                    : <div className="font-serif text-[17px] text-ink">{row.val}</div>}
                  {row.sub && <div className="font-mono text-stone text-sm mt-0.5">{row.sub}</div>}
                </div>
              </motion.div>
            ))}
          ...
          >
            <form onSubmit={submit} className="space-y-5" noValidate>
              {[
                ...
                { name: 'phone',   label: 'Phone / WhatsApp',     type: 'tel',  ph: '+234 800 000 0000' },
                { name: 'service', label: 'Service Type',         type: 'text', ph: 'Airport Transfer · Wedding · Event · Corporate' },
              ].map(fd => (
                <div key={fd.name}>
                  <label htmlFor={fd.name} className="block font-sans text-[9px] tracking-[0.38em] uppercase text-stone mb-2">
                    {fd.label}
                  </label>
                  <input
                    id={fd.name}
              ...
              <div>
                <label htmlFor="message" className="block font-sans text-[9px] tracking-[0.38em] uppercase text-stone mb-2">
                  Message
                </label>
                <textarea
                  id="message"
